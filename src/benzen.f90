! cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
    ...
    subroutine benzen(r_atom,blngth,rorg,iplane,nrotat,iaxis, &
    angle)

! This routine generates the coordianates of six carbon atoms
! forming the benzene backbone.
! cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
IMPLICIT NONE
    ...
    integer::i,i1,i2,i3,irotat,jaxis,j,iatom
    real(kind=8)::pi,sixtyd,r_atom(3,6),blngth,&
...
! =  3 -> z-x plane (atoms 1 and 6 displaced along z-axis)
    ...
    elseif(iplane == 3)then
        ...
        print*,'iplane=',iplane
        print*,'Benzen','Wrong value of iplane'
...
! Atom 2
    ...
    r_atom(i1,2)=-0.5d0*blngth
    r_atom(i2,2)=blngth*dsin(sixtyd)
    r_atom(i3,2)=0.d0

! Atom 3

    r_atom(i1,3)=r_atom(i1,2)
    r_atom(i2,3)=-r_atom(i2,2)
    r_atom(i3,3)=0.d0


! Atom 4

    r_atom(i1,4)=-r_atom(i1,2)
    r_atom(i2,4)=r_atom(i2,2)
    r_atom(i3,4)=0.d0

! Atom 5

    r_atom(i1,5)=-r_atom(i1,3)
    r_atom(i2,5)=r_atom(i2,3)
    r_atom(i3,5)=0.d0

! Atom 6

    r_atom(i1,6)=-r_atom(i1,1)
    r_atom(i2,6)=0.d0
    r_atom(i3,6)=0.d0
